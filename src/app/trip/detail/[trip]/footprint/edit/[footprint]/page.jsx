...
export default function EditFootprintPage() {
  ...
  const { trip: tripId, footprint: footprintId } = useParams(); // Get trip ID from URL parameters
  ...
  const componentName = "Edit Footprint";
  ...
  const submitButtonContent = (
    <>
      {isSubmitting ? (
        ...
        <>
          <Edit className="w-4 h-4" /> Update Footprint
        </>
      )}
    </>
  );
  ...
  useEffect(() => {
    async function loadFootprintData() {
      ...
      try {
        ...
        const formattedDate = footprint.date
          ? new Date(footprint.date).toISOString().slice(0, 16)
          : "";

        // Update form data with existing footprint data
        setFormData({
          title: footprint.title || "",
          description: footprint.description || "",
          location: footprint.location || "",
          location_name: footprint.location_name || "",
          address: footprint.address || "",
          date: formattedDate,
          cover_photo_url: footprint.cover_photo_url,
          latitude: footprint.latitude,
          longitude: footprint.longitude,
          is_public: footprint.is_public,
          trip_id: footprint.trip_id,
        });

        // Set preview image if exists
        if (footprint.cover_photo_url) {
          setPreviewImage(footprint.cover_photo_url);
        }
      } catch (error) {
    ...
    loadFootprintData();
  }, [footprintId, supabase, router, tripId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Failed to update footprint: ${error.message}`);
    } finally {
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl pb-16">
        <div className="mb-8">
          ...
          <p className="text-sm opacity-60">
            Update the details of your journey marker
          </p>
        </div>
